...
export class APIResource {
  ...
  get singular() {
    return this._res.slice(0, -1);
  }
  get lower() {
    return this._res.toLowerCase();
  }
  get upper() {
    return this._res.toUpperCase();
  }
  ...
  public create() {
    return (data: object) => (dispatch: Function) => {
      dispatch({ type: `${this.upper}_CREATING_START` });

      return this._API
        .post(`/${this.lower}`, data)
        .then((json) => {
          dispatch({
  ...
  public update() {
    return (id: string, data: object) => (dispatch: Function) => {
      dispatch({ type: `${this.upper}_UPDATING_START` });

      this._API
        .put(`/${this.lower}/${id}`, data)
        .then((json) => {
          dispatch({
            type: `${this.upper}_UPDATED`,
            [this.singular]: json.data,
            id
          });
  ...
  public remove() {
    return (idOrArray: string | string[]) => (dispatch: Function) => {
      const run = (_id: string) => {
        this._API
          .delete(`/${this.lower}/${_id}`)
          .then(() => {
